...
pub async fn events_for_week(AxumState(state):AxumState<ServerState>,Query(req):Query<QueryParams>)->Result<String,CalendarErrors>{
    let inner_filter=req.inner_filter.ok_or(CalendarErrors::InvalidFilter)?;
    let filter=GetEvents{
        user_id:req.user_id,
        date:req.date,
        filter:TimeFrameFilter::Week(inner_filter)
    };
