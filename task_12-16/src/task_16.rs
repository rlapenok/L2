//функция, которая производит отпрааку значений в канал и возвращает часть канала, отвечающую на прием в вызывающую функцию
...
fn merge(
   ...
   //бесконечный цикл, цитающий поочередно из каналов 
    ...
    //возврат части приемника в вызывающую функцию
    rx
}

fn main() {
    //запуск потока, который отправит слайс в канал
    let a = as_chan(&vec![1, 3, 5, 7]);
    //запуск потока, который отправит слайс в канал

    let b = as_chan(&vec![2, 4, 6, 8]);
    //функция, которая данные из двух канало в одни
    let c = merge(a, b);
    //чтние из полцученного канала 
    for v in c.iter() {
        println!("{v:?}");
    }
}
