use std::{env::{ current_dir, set_current_dir}, io::{stdin, stdout, Write}, path::Path, process::{Child, Command, Stdio}};
    ...
    pub fn run(){
        ...
        loop {
            ...
            while let Some(pipe_cmd) =cmds.next()  {
                ...
                match cmd {
                    Some("cd")=>{
                        if let Err(err) =set_current_dir(Path::new(&args.collect::<Vec<&str>>().join("")))  {
                           ...
                           match cmd {
                               Ok(status)=>{
                                if status.success(){
                                    ...
                                    eprintln!("Killing the process was done with an error :{}",status)
                                }
                        ...
                        if let Some(cmd)=cmd{
                            if piped{
                                let stdin=prev_cmd.map_or(Stdio::inherit(), |out:Child|{
                                  ...
                                  let out=Command::new(cmd).args(args).stdin(stdin).stdout(stdout).spawn();
                                  match out {
                                      Ok(child)=>{
                                            prev_cmd=Some(child)
                                      }
